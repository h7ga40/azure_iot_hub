...
namespace SimEnv
{
	...
	public class EtherIF : IDisposable, IEtherConnection, IUnitInterface
	{
		...
		private static readonly T_IPV6ADDR m_LinkLocal = new T_IPV6ADDR(
			new byte[] { 0xff, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01 }, 0);
		private static Dictionary<IUnitSim, Dictionary<uint, ulong>> m_ArpTables = new Dictionary<IUnitSim, Dictionary<uint, ulong>>();
		private static LinkedList<EtherPacket> m_ResolvingPackets = new LinkedList<EtherPacket>();
		private static EtherIF m_DHCPServer;
		private static ulong[] m_DHCPIPAddrs = new ulong[10];

		public EtherIF(IUnitSim unitSim, string type, string name, int ifKind, ulong macAddr)
		{
			UnitSim = unitSim;
			TypeName = type;
			InterfaceName = name;
			IFKind = ifKind;
			m_MacAddr = macAddr;
			SelfIPv6 = GetIPv6Addr(macAddr);

			if (!m_ArpTables.ContainsKey(UnitSim)) {
				m_ArpTables[UnitSim] = new Dictionary<uint, ulong>();
			}
		...
		private const byte EUI64_UBIT = 0x02;

		private static T_IPV6ADDR GetIPv6Addr(ulong macAddr)
		{
			var addr = BitConverter.GetBytes(macAddr);
		...
		{
			var addr = new byte[T_IPV6ADDR.length];
			var macAddr = new byte[sizeof(ulong)];
		...
		public void Init(uint addr, uint subNet)
		{
			...
			Broadcast = (addr & subNet) | (0xFFFFFFFF & ~subNet);
		}

		public T_IPV6ADDR SelfIPv6 { get; }

		public uint SelfIP { get; private set; }
		...
		public ushort TargetPortNo { get; set; }
		...
		private void CheckAndUpdateArpTable(uint ipAddr, ulong macAddr)
		{
			...
			// IPアドレスのチェック
			if ((SelfIP & SubnetMask) != (ipAddr & SubnetMask)) {
				// サブネットが違う場合は登録しない
				return;
			}

			if ((ipAddr == 0xFFFFFFFF) || (ipAddr == Broadcast)
		...
		{
			m_ArpTables[UnitSim][ipAddr] = macAddr;

			for (var node = m_ResolvingPackets.First;
		...
		{
			var arp = new ARPPacket(packet, EtherPacket.length);

			// 自分宛のIPアドレスの場合
			if (arp.DstPcAddr != SelfIP) {
				return;
			}
			...
			case ArpOperation.Request:
				...
				response = new EtherPacket(buf, 0) {
					Length = buf.Length,
					SourceMac = m_MacAddr,
					DestinationMac = arp.SrcHwAddr,
					TypeLen = EtherPacket.ETHTYPE_ARP
				};

				var res = new ARPPacket(response, EtherPacket.length);
				res.SetArpPacket(ArpOperation.Reply, m_MacAddr, SelfIP, arp.SrcHwAddr, arp.SrcPcAddr);
				break;
			...
			if (buf != null)
				Hub.Send(this, response);
		}

		private void SendArpRequest(uint dstAddr)
		{
			var buf = new byte[EtherPacket.length + ARPPacket.Fields.DstPcAddr.offset + ARPPacket.Fields.DstPcAddr.size];
			var response = new EtherPacket(buf, 0) {
				Length = buf.Length,
				SourceMac = m_MacAddr,
				DestinationMac = 0xFFFFFFFFFFFFUL,
				TypeLen = EtherPacket.ETHTYPE_ARP
			};

			var res = new ARPPacket(response, EtherPacket.length);
			res.SetArpPacket(ArpOperation.Request, m_MacAddr, SelfIP, 0, dstAddr);

			Hub.Send(this, response);
		}

		private const byte ICMP_ER = 0;
		...
		private void ReceiveIcmp(EtherPacket packet)
		{
			...
			case ICMP_ECHO:
				...
				/* adjust the checksum */
				if (ricmp.ICMPChecksum >= lwip.PP_HTONS(0xffff - (ICMP_ECHO << 8)))
					ricmp.ICMPChecksum += (ushort)(lwip.PP_HTONS(ICMP_ECHO << 8) + 1);
				else
					ricmp.ICMPChecksum += lwip.PP_HTONS(ICMP_ECHO << 8);
				ipv4.SetIPCheckSum();
				break;
			}
		...
		private const byte DHCP_DISCOVER = 0x01;

		/// <summary>Offerパケットの指定</summary>
		private const byte DHCP_OFFER = 0x02;

		/// <summary>Requestパケットの指定</summary>
		private const byte DHCP_REQUEST = 0x03;
		private const byte DHCP_DECLINE = 0x04;

		/// <summary>ACKパケットの指定</summary>
		private const byte DHCP_PACK = 0x05;

		/// <summary>NACKパケットの指定</summary>
		private const byte DHCP_PNAK = 0x06;

		/// <summary>Releaseパケットの指定</summary>
		private const byte DHCP_RELEASE = 0x07;
		private const byte DHCP_INFRM = 0x08;

		private void ReceiveDhcp(EtherPacket packet)
		{
			IPv4Packet ipv4 = new IPv4Packet(packet, EtherPacket.length), ripv4;
			UDPPacket udp = new UDPPacket(ipv4, IPv4Packet.length), rudp;
			DHCPPacket dhcp = new DHCPPacket(udp, UDPPacket.length), offer;

			if (dhcp.OP != BOOTREQUEST)
				return;

			var msgType = -1;
			var pos = EtherPacket.length + IPv4Packet.length + UDPPacket.length + DHCPPacket.length + 4;
			int len;
			...
			var buf = new byte[EtherPacket.length + IPv4Packet.length + UDPPacket.length + DHCPPacket.length + len];
			var response = new EtherPacket(buf, 0) {
				Length = buf.Length,
				SourceMac = m_MacAddr,
				DestinationMac = packet.SourceMac,
				TypeLen = EtherPacket.ETHTYPE_IP
			};
			...
			ripv4 = new IPv4Packet(response, EtherPacket.length) {
				...
				TypeOfServices = 0,
				DatagramLength = (ushort)(buf.Length - EtherPacket.length),
				Identification = (ushort)m_Random.Next(UInt16.MaxValue),
				Flags = 2,    // UDPは分割しないでー
				FragmentOffset = 0,
				TimeToLive = 16,  // 寿命
				Protocol = IPv4Packet.IP_PROTO_UDP,
				Source = SelfIP,
			...
			rudp = new UDPPacket(ripv4, IPv4Packet.length) {
				...
				UDPLength = (ushort)(buf.Length - EtherPacket.length - IPv4Packet.length)
			};

			offer = new DHCPPacket(rudp, UDPPacket.length);
			var macaddr = BitConverter.GetBytes(m_MacAddr);
			...
			buf[pos++] = 0x01;
			buf[pos++] = 0x04;
			buf[pos++] = (byte)((SubnetMask >> 24) & 0xFF);
			buf[pos++] = (byte)((SubnetMask >> 16) & 0xFF);
			buf[pos++] = (byte)((SubnetMask >> 8) & 0xFF);
			buf[pos++] = (byte)(SubnetMask & 0xFF);
			// Router Option
			buf[pos++] = 0x03;
			buf[pos++] = 0x04;
			...
			buf[pos++] = 0x36;
			buf[pos++] = 0x04;
			buf[pos++] = (byte)((SelfIP >> 24) & 0xFF);
			buf[pos++] = (byte)((SelfIP >> 16) & 0xFF);
			buf[pos++] = (byte)((SelfIP >> 8) & 0xFF);
			buf[pos++] = (byte)(SelfIP & 0xFF);
			/* Stoper */
			buf[pos++] = 0xFF;
		...
		{
			var smac = ((ulong)array[0] << 40) + ((ulong)array[1] << 32) + ((ulong)array[2] << 24)
				+ ((ulong)array[3] << 16) + ((ulong)array[4] << 8) + array[5];

			for (var i = 0; i < m_DHCPIPAddrs.Length; i++) {
				var mac = m_DHCPIPAddrs[i];
				if (mac == smac)
					return SelfIP + (uint)i + 1u;
			}
			for (var i = 0; i < m_DHCPIPAddrs.Length; i++) {
				var mac = m_DHCPIPAddrs[i];
				if (mac == 0) {
		...
		private void ReceiveIPv4(EtherPacket packet)
		{
			...
			// IPアドレスのチェック
			if ((ipv4.Destination != SelfIP) && (ipv4.Destination != Broadcast)
				 && !multicast)
				return;
			...
			var len = packet.Length - EtherPacket.length - IPv4Packet.length;
			if (len < 0)
				return;

			var data = Itron.GetIp4Packet(new pointer(ipv4, IPv4Packet.length), len,
		...
		{
			var ipv6 = new IPv6Packet(packet, EtherPacket.length);
			var multicast = false;
			...
			// IPアドレスのチェック
			if ((pointer.memcmp(ipv6.DestinationAddress, SelfIPv6, T_IPV6ADDR.length) != 0)
			...
			var len = packet.Length - EtherPacket.length - IPv6Packet.length;
			if (len < 0)
				return;

			var data = Itron.GetIp4Packet(new pointer(ipv6, IPv6Packet.length), len,
		...
		{
			var ipv6 = new IPv6Packet(packet, EtherPacket.length);
			var icmp = new ICMPv6Packet(ipv6, IPv6Packet.length);

			switch (icmp.ICMPType) {
			case 135:
				var ns = new NeighborSolicitationPacket(icmp, ICMPv6Packet.length);
				if (pointer.memcmp(ns.TargetAddress, SelfIPv6, T_IPV6ADDR.length) != 0)
					break;

				ushort len = ICMPv6Packet.length + NeighborAdvertisementPacket.length + ICMPv6OptionPacket.length + EthernetAddressPacket.length;
				var data = new byte[EtherPacket.length + IPv6Packet.length + len];
				var response = new EtherPacket(data, 0);
				var ripv6 = new IPv6Packet(response, EtherPacket.length);
				var ricmp = new ICMPv6Packet(ripv6, IPv6Packet.length);
				var rna = new NeighborAdvertisementPacket(ricmp, ICMPv6Packet.length);
				var ropt = new ICMPv6OptionPacket(rna, NeighborAdvertisementPacket.length);
				var rlla = new EthernetAddressPacket(ropt, ICMPv6OptionPacket.length);
				...
				ripv6.HopLimit = 255;
				pointer.memcpy(ripv6.SourceAddress, SelfIPv6, T_IPV6ADDR.length);
				...
				rna.OverrideFlag = 1;
				rna.Reserved = 0;
				pointer.memcpy(rna.TargetAddress, SelfIPv6, T_IPV6ADDR.length);
				ropt.Type = 2;
				ropt.Length = 1;
				var macAddr = BitConverter.GetBytes(m_MacAddr);
				Array.Reverse(macAddr);
		...
		public void Push(byte[] data)
		{
			if (data[0] == 4) {
				...
				ipv4 = Itron.CastIp4Packet(data, out var len, out var src, out var dst, out var proto);
				...
				ipv6 = Itron.CastIp6Packet(data, out var len, out var src, out var dst, out var proto);
		...
		private void Push4(pointer data, int len, ip_addr src, ip_addr dst, byte proto)
		{
			...
			// ブロードキャストの場合
			else if ((dst.addr == 0xFFFFFFFF) || ((dst.addr & SubnetMask) == ~SubnetMask)) {
				dmac = 0xFFFFFFFFFFFFUL;
			}
			// IPアドレスでARPテーブルから検索
			else if (!m_ArpTables[UnitSim].TryGetValue(dst.addr, out dmac)) {
				dmac = 0;
				resolve = true;
			}

			var buf = new byte[EtherPacket.length + IPv4Packet.length + len];
			...
			var ipv4 = new IPv4Packet(packet, EtherPacket.length) {
				...
				InternetHeaderLength = 5,
				TypeOfServices = 0,
				DatagramLength = (ushort)(len + IPv4Packet.length),
				Identification = (ushort)m_Random.Next(UInt16.MaxValue),
				Flags = 2, // UDPは分割しないでー
				FragmentOffset = 0,
				TimeToLive = 16,   // 寿命
		...
		private void Push6(pointer data, int len, ip6_addr src, ip6_addr dst, byte proto)
		{
			...
			var buf = new byte[EtherPacket.length + IPv6Packet.length + len];

			var packet = new EtherPacket(buf, 0) {
				Length = buf.Length,
				DestinationMac = dmac,
				SourceMac = m_MacAddr,
				TypeLen = EtherPacket.ETHTYPE_IPV6
			};
			...
			var ipv6 = new IPv6Packet(packet, EtherPacket.length) {
				...
				TrafficClass = 0,
				FlowLabel = 0,
				PaylaodLength = (ushort)len,
				NextHeader = proto,
				HopLimit = 1
			};
			pointer.memcpy(ipv6.SourceAddress, SelfIPv6, T_IPV6ADDR.length);
		...
		{
			private ip6_addr m_TargetAddr = new ip6_addr(new byte[16]);
			private ulong m_MacAddr;
			private bool m_IsRouter;
			public enum TState
			{
				/// <summary>不完全</summary>
				INCOMPLETE,
			...
			private Queue<EtherPacket> m_Queue = new Queue<EtherPacket>();
			private EtherIF m_Owner;
			...
			public ip6_addr TargetAddr => m_TargetAddr;

			public ulong MacAddr => m_MacAddr;

			public bool IsRouter => m_IsRouter;
			...
			private void SendNeighborSolicitation()
			{
				...
				var ripv6 = new IPv6Packet(response, EtherPacket.length);
				var ricmp = new ICMPv6Packet(ripv6, IPv6Packet.length);
				var rna = new NeighborSolicitationPacket(ricmp, ICMPv6Packet.length);
				var ropt = new ICMPv6OptionPacket(rna, NeighborSolicitationPacket.length);
				var rlla = new EthernetAddressPacket(ropt, ICMPv6OptionPacket.length);
				...
				ripv6.NextHeader = IPv6Packet.IP_PROTO_ICMPV6;
				ripv6.HopLimit = 255;
				pointer.memcpy(ripv6.SourceAddress, m_Owner.SelfIPv6, T_IPV6ADDR.length);
			...
			public void Receive(EtherPacket packet)
			{
				...
				if (pointer.memcmp(ipv6.DestinationAddress, m_Owner.SelfIPv6, T_IPV6ADDR.length) != 0)
					return;
				...
				var na = new NeighborAdvertisementPacket(icmp, ICMPv6Packet.length);

				if (pointer.memcmp(na.TargetAddress, m_Owner.SelfIPv6, T_IPV6ADDR.length) != 0)
					return;

				int len, end = ipv6.PaylaodLength;
				var pos = ICMPv6Packet.length + NeighborAdvertisementPacket.length;
				for (var opt = new ICMPv6OptionPacket(na, pos); pos < end;
					len = 8 * opt.Length, pos += len, opt = new ICMPv6OptionPacket(na, pos)) {
					if ((opt.Type != 2) || (opt.Length != 1))
						continue;

					var lla = new EthernetAddressPacket(opt, ICMPv6OptionPacket.length);
					var macAddr = new byte[sizeof(uint)];
