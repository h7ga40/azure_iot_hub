...
namespace MicroServer.Serializers.Json
{
		...
		: JToken
	{
		...
		public object RawValue => BoxedValue;
		...
		internal override void Serialize(StringBuilder sb)
		{
			if (BoxedValue == null) {
				...
				sb.Append((bool)BoxedValue ? "true" : "false");
			}
			else if (BoxedValue is double) {
#if (MF_FRAMEWORK_VERSION_V4_2 || MF_FRAMEWORK_VERSION_V4_3)
                sb.Append(((double)this.BoxedValue).ToString());
#else
				sb.Append(((double)BoxedValue).ToString(CultureInfo.InvariantCulture));
#endif
			}
			else if (BoxedValue is string) {
				sb.Append('"');
				sb.Append((string)BoxedValue);
				sb.Append('"');
			}
			else {
		...
		: JToken, IEnumerable
	{
		...
		public void Add(string name, JToken value)
		{
			...
			if (_itemCount >= _items.Length) {
				...
				Array.Copy(
					old,
					_items = new JProperty[_itemCount + ChunkSize],
					_itemCount
					);
			}

			_items[_itemCount++] = new JProperty(name, value);
		}
