...
namespace MicroServer.Net.Http
{
	public class HttpService : SocketService
	{
		...
		/// </summary>
		public int ActivePort => _listener.ActivePort;
		/// <summary>
		/// Gets current server.
		/// </summary>
		/// <remarks>
		...
		/// </remarks>
		public static HttpService Current => _service;
		...
		public override bool Start()
		{
			try {
				_listener = new TcpListener {
					...
					SendTimeout = Constants.HTTP_SEND_TIMEOUT,
					ListenBacklog = Constants.HTTP_LISTEN_BACKLOG
				};
		...
		private void SendChannel(SocketChannel channel, object message)
		{
			...
			try {
				var sendMessage = _encoder.Prepare(message);
		...
		private void SendCompleted(int bytesTransferred)
		{
			...
			var isComplete = _encoder.SendCompleted(bytesTransferred);
			if (!isComplete) {
				return;
			}
		}
		...
		private void SendFailure(SocketChannel channel, Exception ex)
		{
			...
			var response = Encoding.UTF8.GetBytes("HTTP/1.1 400 OK\r\n" +
