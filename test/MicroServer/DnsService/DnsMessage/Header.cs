...
namespace MicroServer.Net.Dns
{
	...
	public class Header
	{
		...
		/// </summary>
		public bool QR {
			get => ByteUtility.GetBits(_flags, 15, 1) == 0;
			set {
				if (value == true) {
		...
		{
			get => (OperationCode)ByteUtility.GetBits(_flags, 11, 4);
		...
		{
			get => ByteUtility.GetBit(_flags, 10) == true;
		...
		{
			get => ByteUtility.GetBit(_flags, 9) == true;
		...
		{
			get => ByteUtility.GetBit(_flags, 8) == true;
		...
		{
			get => ByteUtility.GetBit(_flags, 7) == true;
		...
		private ushort ZReserved {
			get => ByteUtility.GetBits(_flags, 4, 3);
		...
		{
			get => (ReturnCode)ByteUtility.GetBits(_flags, 0, 4);
