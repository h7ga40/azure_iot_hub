...
namespace uITron3
{
	...
	public abstract class Kernel : IKernel
	{
		...
		protected List<PacketBridge> m_PacketBridges = new List<PacketBridge>();
		LinkedList<PacketBridge> m_PacketEvent = new LinkedList<PacketBridge>();
		...
		protected abstract bool InterruptUnmasked(int intNo);
		...
		public bool InterruptRaised(int intNo) { return m_InProcIntr[intNo]; }
		...
		void MainLoop()
		{
			...
			do {
				...
				while (m_PacketEvent.Count > 0) {
					PacketBridge pb;
					lock (m_PacketEvent) {
						pb = m_PacketEvent.First.Value;
						m_PacketEvent.RemoveFirst();
					}
					pb.DoInputData();
				}
		...
		public void Interrupt(PacketBridge pb)
		{
			lock (m_PacketEvent) {
				m_PacketEvent.AddLast(pb);
			}
			m_IntEvent.Set();
		}
