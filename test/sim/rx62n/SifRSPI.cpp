...
//	戻り値：
//------------------------------------------------------------------------------
CSifRSPI::CSifRSPI(TRX62NRSPI0Reg *reg, int rxIntNo, int txIntNo, int idIntNo, int devNo)
{
	m_Reg = reg;
	m_TxIntNo = txIntNo;
	m_RxIntNo = rxIntNo;
...
uint8_t CSifRSPI::GetByte(intptr_t index)
{
	...
	// 受信バッファが読み込まれたら
	if((index >= (intptr_t)&((TRX62NRSPI0Reg *)0)->SPDR)
		&& (index <= (intptr_t)&(&((TRX62NRSPI0Reg *)0)->SPDR)[1])){
		return ((uint8_t *)&m_SPRX)[index - (intptr_t)&((TRX62NRSPI0Reg *)0)->SPDR];
	}
...
uint32_t CSifRSPI::GetInt32(intptr_t index)
{
	...
	// 受信バッファが読み込まれたら
	if(index == (uintptr_t)&((TRX62NRSPI0Reg *)0)->SPDR){
		return m_SPRX;
	}
