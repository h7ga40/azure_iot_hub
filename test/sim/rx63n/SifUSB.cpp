...
CSifUSB::CSifUSB(TRX63NUSB0Reg *usbReg, int d0FifoIntNo, int d1FifoIntNo,
...
unsigned char CSifUSB::GetByte(unsigned int Addr)
{
	...
	{
	// CFIFOポートレジスタ
	case offsetof(TRX63NUSB0Reg, CFIFO):
		Value = m_CFifo.GetByte();
		break;
	// D0FIFOポートレジスタ
	case offsetof(TRX63NUSB0Reg, D0FIFO):
		Value = m_D0Fifo.GetByte();
		break;
	// D1FIFOポートレジスタ
	case offsetof(TRX63NUSB0Reg, D1FIFO):
...
unsigned short CSifUSB::GetUInt16(unsigned int Addr)
{
	...
	case offsetof(TRX63NUSB0Reg, SYSCFG):
		break;
	// システムコンフィギュレーションステータスレジスタ0
	case offsetof(TRX63NUSB0Reg, SYSSTS0):
		break;
	// デバイスステートコントロールレジスタ0
	case offsetof(TRX63NUSB0Reg, DVSTCTR0):
		break;
	// CFIFOポートレジスタ
	case offsetof(TRX63NUSB0Reg, CFIFO):
	...
	case offsetof(TRX63NUSB0Reg, D1FIFO):
		...
		break;
	// CFIFOポート選択レジスタ
	case offsetof(TRX63NUSB0Reg, CFIFOSEL):
		Value = *(unsigned short *)&m_CFifo.GetFIFOSEL();
		break;
	// CFIFOポートコントロールレジスタ
	case offsetof(TRX63NUSB0Reg, CFIFOCTR):
		Value = *(unsigned short *)&m_CFifo.GetFIFOCTR();
		break;
	// D0FIFOポート選択レジスタ
	case offsetof(TRX63NUSB0Reg, D0FIFOSEL):
		Value = *(unsigned short *)&m_D0Fifo.GetFIFOSEL();
		break;
	// D0FIFOポートコントロールレジスタ
	case offsetof(TRX63NUSB0Reg, D0FIFOCTR):
		Value = *(unsigned short *)&m_D0Fifo.GetFIFOCTR();
		break;
	// D1FIFOポート選択レジスタ
	case offsetof(TRX63NUSB0Reg, D1FIFOSEL):
		Value = *(unsigned short *)&m_D1Fifo.GetFIFOSEL();
		break;
	// D1FIFOポートコントロールレジスタ
	case offsetof(TRX63NUSB0Reg, D1FIFOCTR):
		Value = *(unsigned short *)&m_D1Fifo.GetFIFOCTR();
		break;
	// 割り込み許可レジスタ0
	case offsetof(TRX63NUSB0Reg, INTENB0):
		break;
	// 割り込み許可レジスタ1
	case offsetof(TRX63NUSB0Reg, INTENB1):
		break;
	// BRDY割り込み許可レジスタ
	case offsetof(TRX63NUSB0Reg, BRDYENB):
		break;
	// NRDY割り込み許可レジスタ
	case offsetof(TRX63NUSB0Reg, NRDYENB):
		break;
	// BEMP割り込み許可レジスタ
	case offsetof(TRX63NUSB0Reg, BEMPENB):
		break;
	// SOF出力コンフィグレーションレジスタ
	case offsetof(TRX63NUSB0Reg, SOFCFG):
		break;
	// 割り込みステータスレジスタ0
	case offsetof(TRX63NUSB0Reg, INTSTS0):
		break;
	// 割り込みステータスレジスタ1
	case offsetof(TRX63NUSB0Reg, INTSTS1):
		break;
	// BRDY割り込みステータスレジスタ
	case offsetof(TRX63NUSB0Reg, BRDYSTS):
		break;
	// NRDY割り込みステータスレジスタ
	case offsetof(TRX63NUSB0Reg, NRDYSTS):
		break;
	// BEMP割り込みステータスレジスタ
	case offsetof(TRX63NUSB0Reg, BEMPSTS):
		break;
	// フレームナンバレジスタ
	case offsetof(TRX63NUSB0Reg, FRMNUM):
		break;
	// デバイスステート切り替えレジスタ
	case offsetof(TRX63NUSB0Reg, DVCHGR):
		break;
	// USBアドレスレジスタ
	case offsetof(TRX63NUSB0Reg, USBADDR):
		break;
	// USBリクエストタイプレジスタ
	case offsetof(TRX63NUSB0Reg, USBREQ):
		break;
	// USBリクエストバリューレジスタ
	case offsetof(TRX63NUSB0Reg, USBVAL):
		break;
	// USBリクエストインデックスレジスタ
	case offsetof(TRX63NUSB0Reg, USBINDX):
		break;
	// USBリクエストレングスレジスタ
	case offsetof(TRX63NUSB0Reg, USBLENG):
		break;
	// DCPコンフィギュレーションレジスタ
	case offsetof(TRX63NUSB0Reg, DCPCFG):
		Value = *(unsigned short *)&m_UsbPipe[0].GetPIPECFG();
		break;
	// DCPマックスパケットサイズレジスタ
	case offsetof(TRX63NUSB0Reg, DCPMAXP):
		Value = *(unsigned short *)&m_UsbPipe[0].GetPIPEMAXP();
		break;
	// DCPコントロールレジスタ
	case offsetof(TRX63NUSB0Reg, DCPCTR):
		Value = *(unsigned short *)&m_UsbPipe[0].GetPIPECTR();
		break;
	// パイプウィンドウ選択レジスタ
	case offsetof(TRX63NUSB0Reg, PIPESEL):
		break;
	// パイプコンフィギュレーションレジスタ
	case offsetof(TRX63NUSB0Reg, PIPECFG):
		...
		break;
	// パイプマックスパケットサイズレジスタ
	case offsetof(TRX63NUSB0Reg, PIPEMAXP):
		if(m_UsbRegInst.PIPESEL.PIPESEL == 0){
			Value = 0;
		}
		else{
		...
		break;
	// パイプ周期制御レジスタ
	case offsetof(TRX63NUSB0Reg, PIPEPERI):
		if(m_UsbRegInst.PIPESEL.PIPESEL == 0){
			Value = 0;
		}
		else{
