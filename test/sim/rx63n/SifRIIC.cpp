...
//	戻り値：
//------------------------------------------------------------------------------
CSifRIIC::CSifRIIC(TRX63NRIIC0Reg *reg, int txIntNo, int rxIntNo, int teIntNo, int devNo)
{
	m_Reg = reg;
	m_TxIntNo = txIntNo;
	m_RxIntNo = rxIntNo;
...
unsigned char CSifRIIC::GetByte(intptr_t index)
{
	...
	if (index == (intptr_t)&((TRX63NRIIC0Reg *)0)->ICCR2) {
		return m_RegInst.ICCR2;
	}
	// 受信バッファが読み込まれたら
	else if (index == (uintptr_t)&((TRX63NRIIC0Reg *)0)->ICDRR) {
		InterlockedAnd8((char *)&m_RegInst.ICSR2, ~0x20);	// RDRF
	}
	else if (index == (intptr_t)&((TRX63NRIIC0Reg *)0)->ICSR2) {
...
void CSifRIIC::SetByte(intptr_t index, unsigned char value)
{
	...
	// 送信バッファに書き込まれたら
	if (index == (intptr_t)&((TRX63NRIIC0Reg *)0)->ICDRT) {
		// TDRE
