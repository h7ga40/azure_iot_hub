...
//	戻り値：
//------------------------------------------------------------------------------
CSifSCI::CSifSCI(TRX63NSCI0Reg *reg, int txIntNo, int rxIntNo, int teIntNo, int devNo)
{
	m_Reg = reg;
	m_TxIntNo = txIntNo;
	m_RxIntNo = rxIntNo;
...
unsigned char CSifSCI::GetByte(intptr_t index)
{
	...
	// 受信バッファが読み込まれたら
	if (index == (uintptr_t)&((TRX63NSCI0Reg *)0)->RDR) {
#ifdef SIF_SCI_TYPE2
		// このブロックの終わりまで割り込み禁止
		Lock lock(this);
	...
	else if (index == (intptr_t)&((TRX63NSCI0Reg *)0)->SSR) {
...
void CSifSCI::SetByte(intptr_t index, unsigned char value)
{
	...
	// 送信バッファに書き込まれたら
	if (index == (intptr_t)&((TRX63NSCI0Reg *)0)->TDR) {
		InterlockedAnd8((char *)&m_RegInst.SSR, ~0x80);
		if ((InterlockedAnd8((char *)&m_RegInst.SSR, ~0x04) & 0x04) != 0)
			g_Kernel->OnSetEvent();
	}
}
...
{
	if ((m_SendTerm == scistIdle) && ((m_RegInst.SSR & 0x04) == 0)) {
		m_SendTimer = 0;
		m_SendTerm = scistIdle;
	}
	else if ((m_SendTerm == scistIdle) && ((m_RegInst.SSR & 0x80) != 0)
		&& (m_RegInst.SCR & 0x04) != 0) {
		m_SendTimer = 0;
