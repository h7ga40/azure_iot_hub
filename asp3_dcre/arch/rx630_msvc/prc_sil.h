 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: prc_sil.h 2049 2020-01-22 10:38:36Z coas-nagasima $
 */
...
extern uint8_t sil_reb_mem(const volatile uint8_t *mem);

extern void sil_wrb_mem(volatile uint8_t *mem, uint8_t data);
...
extern uint16_t sil_reh_mem(const volatile uint16_t *mem);

extern void sil_wrh_mem(volatile uint16_t *mem, uint16_t data);


#define	sil_reh_lem(mem)		sil_reh_mem(mem)
#define	sil_wrh_lem(mem, data)	sil_wrh_mem(mem, data)

extern uint16_t sil_reh_bem(const volatile uint16_t *mem);
extern void sil_wrh_bem(volatile uint16_t *mem, uint16_t data);

/*
 *  32ビット単位の読出し／書込み
 */

extern uint32_t sil_rew_mem(const volatile uint32_t *mem);
extern void sil_wrw_mem(volatile uint32_t *mem, uint32_t data);

#define	sil_rew_lem(mem)		sil_rew_mem(mem)
#define	sil_wrw_lem(mem, data)	sil_wrw_mem(mem, data)

extern uint32_t sil_rew_bem(const volatile uint32_t *mem);
extern void sil_wrw_bem(volatile uint32_t *mem, uint32_t data);
