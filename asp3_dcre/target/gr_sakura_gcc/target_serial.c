...
sio_opn_por(ID siopid, intptr_t exinf)
{
	...
	if (!(p_siopcb->opened)) {
		if (p_siopinib->path != NULL) {
			fd = siofd_open(p_siopinib->path, O_RDWR, 0777);
			assert(fd >= 0);
			p_siopcb->read_fd = fd;
		...
		siofd_tcgetattr(fd, &(p_siopcb->saved_term));
...
sio_cls_por(SIOPCB *p_siopcb)
{
	...
	if (p_siopcb->opened) {
		fd = p_siopcb->read_fd;
		siofd_tcsetattr(fd, TCSAFLUSH, &(p_siopcb->saved_term));
		siofd_fcntl(fd, F_SETFL, 0);

		if (p_siopcb->p_siopinib->path != NULL) {
			siofd_close(p_siopcb->read_fd);
		}
...
sio_isr_snd(ID siopid)
{
	...
	if (p_siopcb->snd_flag) {
		if ((n = siofd_write(p_siopcb->write_fd, &(p_siopcb->snd_buf), 1)) > 0) {
			p_siopcb->snd_flag = false;
			if (p_siopcb->snd_rdy) {
				sio_irdy_snd(p_siopcb->exinf);
				return p_siopcb->snd_rdy;
			}
		}
	}
...
sio_snd_chr(SIOPCB *p_siopcb, char c)
{
	...
	if (!p_siopcb->snd_flag) {
		if ((n = siofd_write(p_siopcb->write_fd, &c, 1)) > 0) {
			return(true);
		}
		else {
...
sio_rcv_chr(SIOPCB *p_siopcb)
{
	...
	else if ((n = siofd_read(p_siopcb->read_fd, &c, 1)) > 0) {
